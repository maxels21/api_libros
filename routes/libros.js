...
router.post('/', (req, res, next) => {
    try {
        ...
        if (error) {
            ...
            validationError.details = error.details.map(details => detail.message);
...
router.put('/:id', (req, res, next) => {
    try { 
        ...
        if (error) {
            ...
            validationError.status = 400;
            validationError.details = error.details.map(detail => detailmessage);
            throw validationError;
        }
...
router.delete('/:id', (req, res, next) => {
    try {
        ...
        if (index == -1) {
            const error = new Error('Libros no encontrado');
            error.status = 404;
            throw error;
        } 
